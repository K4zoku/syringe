...
static inline uintptr_t syringe_hook_page_floor(uintptr_t a) { return a & ~((uintptr_t)getpagesize() - 1); }
...
static inline int syringe_hook_process_vm_writev(void *dst, const void *src, size_t len) {
  ...
  ssize_t ret = syscall(__NR_process_vm_writev, getpid(), &rem_iov, 1, &loc_iov, 1, 0);
...
static inline int syringe_hook_safe_write(void *dst, const void *src, size_t len) {
  ...
  if (ro) {
    ...
    SYRINGE_HOOK_LOG("safe_write: ALL methods failed for %p (mprotect err=%d, memfd, pvwrite)", dst, merr);
    return -1;
  }
  memcpy(dst, src, len);
  return 0;
...
static inline int syringe_hook_safe_write_ptr(void **slot, void *val) {
  ...
  if (ro) {
    ...
    int merr = errno;
    if (syringe_hook_mem_write(slot, &val, sizeof(val)) == 0)
      return 0;
    if (syringe_hook_process_vm_writev(slot, &val, sizeof(val)) == 0)
      return 0;
    SYRINGE_HOOK_LOG("safe_write_ptr: ALL methods failed for %p (mprotect err=%d)", (void *)slot, merr);
    return -1;
  }
  syringe_hook_atomic_write_ptr(slot, val);
  return 0;
...
static inline int syringe_hook_tramp_install(Trampoline *t, void *target, void *hook, void **orig_out) {
  ...
  t->bounce =
      (uint8_t *)mmap(NULL, t->bounce_len, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  if (t->bounce == MAP_FAILED) {
    t->bounce = (uint8_t *)mmap(NULL, t->bounce_len, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    if (t->bounce == MAP_FAILED) {
      SYRINGE_HOOK_LOG("tramp: mmap bounce failed: %s", strerror(errno));
      return -1;
    }
    if (mprotect(t->bounce, t->bounce_len, PROT_READ | PROT_EXEC) != 0) {
      SYRINGE_HOOK_LOG("tramp: mprotect bounce RX failed: %s", strerror(errno));
  ...
  size_t stolen = syringe_hook_arch_tramp_make(t->bounce, t->bounce_len, target, &t->stolen_len);
  if (stolen == 0) {
  ...
  if (syringe_memfd >= 0) {
    ssize_t rd = pread(syringe_memfd, t->stolen, t->stolen_len, (off_t)(uintptr_t)target);
  ...
  __builtin___clear_cache((char *)t->bounce, (char *)t->bounce + t->bounce_len);
  ...
  __builtin___clear_cache((char *)target, (char *)target + TRAMP_JMP_SZ);

  t->active = 1;
  if (orig_out)
    *orig_out = t->bounce;
  SYRINGE_HOOK_LOG("tramp: patched %p → %p (stolen=%zu, bounce @ %p)", target, hook, t->stolen_len, t->bounce);
  return 0;
}
...
static inline void syringe_hook_tramp_remove(Trampoline *t) {
  ...
  __builtin___clear_cache((char *)t->target, (char *)t->target + t->stolen_len);
...
static inline void syringe_hook_force_resolve(const char *sym) { (void)dlsym(RTLD_DEFAULT, sym); }

static inline void syringe_hook_patch_rela(const Elf64_Rela *rela, size_t n, const Elf64_Sym *symtab,
                                           const char *strtab, uintptr_t bias, WalkCtx *ctx) {
  for (size_t i = 0; i < n; i++) {
    ...
    if (sidx == STN_UNDEF)
      continue;
    if (strcmp(strtab + symtab[sidx].st_name, ctx->sym_name) != 0)
      continue;
...
static inline void syringe_hook_patch_rel(const Elf64_Rel *rel, size_t n, const Elf64_Sym *symtab, const char *strtab,
...
static inline int syringe_hook_phdr_cb(struct dl_phdr_info *info, size_t sz, void *data) {
  ...
  const Elf64_Rel *rel_plt = NULL;
  size_t rel_plt_sz = 0;
  const Elf64_Rel *rel_dyn = NULL;
  size_t rel_dyn_sz = 0;
  ...
  for (const Elf64_Dyn *d = dyn; d->d_tag != DT_NULL; d++) {
    switch (d->d_tag) {
    case DT_SYMTAB:
      symtab = (const Elf64_Sym *)d->d_un.d_ptr;
      break;
    case DT_STRTAB:
      strtab = (const char *)d->d_un.d_ptr;
      break;
    case DT_JMPREL:
      rela_plt = (const Elf64_Rela *)d->d_un.d_ptr;
      break;
    case DT_PLTRELSZ:
      rela_plt_sz = d->d_un.d_val;
      break;
    case DT_PLTREL:
      pltrel_is_rela = (d->d_un.d_val == DT_RELA);
      break;
    case DT_RELA:
      rela_dyn = (const Elf64_Rela *)d->d_un.d_ptr;
      break;
    case DT_RELASZ:
      rela_dyn_sz = d->d_un.d_val;
      break;
    case DT_REL:
      rel_dyn = (const Elf64_Rel *)d->d_un.d_ptr;
      break;
    case DT_RELSZ:
      rel_dyn_sz = d->d_un.d_val;
      break;
    default:
      break;
    }
  ...
  if (pltrel_is_rela) {
    if (rela_plt && rela_plt_sz)
      syringe_hook_patch_rela(rela_plt, rela_plt_sz / sizeof(Elf64_Rela), symtab, strtab, bias, ctx);
  } else {
    rel_plt = (const Elf64_Rel *)rela_plt;
    rel_plt_sz = rela_plt_sz;
    if (rel_plt && rel_plt_sz)
      syringe_hook_patch_rel(rel_plt, rel_plt_sz / sizeof(Elf64_Rel), symtab, strtab, bias, ctx);
  }

  if (rela_dyn && rela_dyn_sz)
    syringe_hook_patch_rela(rela_dyn, rela_dyn_sz / sizeof(Elf64_Rela), symtab, strtab, bias, ctx);
  if (rel_dyn && rel_dyn_sz)
    syringe_hook_patch_rel(rel_dyn, rel_dyn_sz / sizeof(Elf64_Rel), symtab, strtab, bias, ctx);

  if (ctx->count > before) {
    const char *mod = (info->dlpi_name && info->dlpi_name[0]) ? info->dlpi_name : "(main)";
...
static inline int syringe_hook_install(const char *sym, void *hook, void **orig_out) {
  ...
  if (rec->orig_addr == NULL || rec->orig_addr == hook) {
    SYRINGE_HOOK_LOG("'%s': orig_addr is NULL=%d or ==hook=%d, falling back to dlsym", sym, rec->orig_addr == NULL,
                     rec->orig_addr == hook);
...
static inline int syringe_hook_install_addr(const char *sym, void *target, void *hook, void **orig_out) {
  if (syringe_nhooks >= SYRINGE_HOOK_MAX) {
    SYRINGE_HOOK_LOG("hook table full");
    return 0;
  }
  ...
  SYRINGE_HOOK_LOG("'%s': install_addr OK (target=%p, bounce=%p)", sym, target, tramp_orig);
  syringe_nhooks++;
  return 1;
}

#ifndef SYRINGE_HOOK_NO_HELPERS
static inline void *syringe_hook_jmp_target(void *src) { return syringe_hook_arch_read_jmp_target(src); }

static inline int syringe_hook_remove(const char *sym) {
  for (int i = 0; i < syringe_nhooks; i++) {
    SyringeHookRecord *rec = &syringe_hooks[i];
    if (strcmp(rec->sym, sym) != 0)
      continue;
    ...
    if (rec->has_tramp)
      syringe_hook_tramp_remove(&rec->tramp);
...
static inline int syringe_hook_count(void) { return syringe_nhooks; }
...
static inline int syringe_hook_registry_size(void) { return SYRINGE_HOOK_MAX; }
