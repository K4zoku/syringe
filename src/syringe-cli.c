...
static int check_ptrace_capability(pid_t target_pid) {
  ...
  if (has_cap_sys_ptrace) {
    ...
    if (syringe_verbose)
      fprintf(stderr,
              "[+] CAP_SYS_PTRACE present — ptrace will work "
  ...
  if (yama_scope >= 3) {
    fprintf(stderr,
            "[!] Kernel yama ptrace_scope=%d (no ptrace at all) "
  ...
  if (yama_scope == 1) {
    ...
    if (sf) {
      ...
      pid_t target_ppid = 0;
      while (fgets(line, sizeof(line), sf)) {
        if (sscanf(line, "PPid: %d", &target_ppid) == 1)
          break;
      }
      fclose(sf);
      if (target_ppid == getpid()) {
        if (syringe_verbose)
          fprintf(stderr,
                  "[+] yama ptrace_scope=1 — we are parent of "
    ...
    fprintf(stderr,
            "    If injection fails with EPERM, run as root or: "
            "sudo setcap cap_sys_ptrace+ep %s\n",
            program_invocation_name);
