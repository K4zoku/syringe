...
static inline void syringe_hook_build_jmp(uint8_t *buf, void *dest) { syringe_hook_arch_build_jmp(buf, dest); }
...
static inline void *syringe_hook_arch_read_jmp_target(void *src) {
  ...
  if (p[0] != 0xFF || p[1] != 0x25)
    return NULL;
  if (p[2] != 0x00 || p[3] != 0x00 || p[4] != 0x00 || p[5] != 0x00)
    return NULL;
  void *dst;
...
static inline size_t syringe_hook_arch_disasm(const uint8_t *code, int *reloc_offset) {
  enum {
    F_MODRM = 1 << 0,
  ...
  for (size_t i = 0; i < sizeof(legacy_prefixes) / sizeof(*legacy_prefixes); i++) {
    if (code[len] == legacy_prefixes[i]) {
      ...
      if (len >= 15)
        return 0; /* x86 caps prefix count */
    }
  }
  ...
  if (len >= 1 && code[len] == 0x0F && code[len + 1] == 0x1E && (code[len + 2] == 0xFA || code[len + 2] == 0xFB)) {
    len += 3;
    return len;
  }
  ...
  if (code[len] == 0x0F) {
    ...
    if (op2 == 0x28 || op2 == 0x29 || op2 == 0x10 || op2 == 0x11 || op2 == 0x57 /* XORPS */) {
      uint8_t modrm = code[len + 2];
      int mod = modrm >> 6;
      int rm = modrm & 7;
      size_t insn_len = 3;
  ...
  for (size_t i = 0; i < sizeof(opcodes) / sizeof(*opcodes); i++) {
    ...
    if (o->flags & F_PLUS_R) {
      ...
      if ((code[len] & 0xF8) == o->opcode)
        found = 1;
    } else if (o->flags & F_REG_OPCODE) {
      /* exact opcode + ModRM /n must match */
      if (code[len] == o->opcode && ((code[len + 1] >> 3) & 7) == o->reg_opcode)
        found = 1;
    } else {
  ...
  /* 8) Immediates. */
  if (match->flags & F_IMM8)
    len += 1;
  if (match->flags & F_IMM16)
    len += 2;
  if (match->flags & F_IMM32)
    len += (has_66 ? 2 : 4);
...
/* Backward-compat alias */
static inline size_t syringe_hook_disasm_x86_64(const uint8_t *code, int *reloc_offset) {
 ...
 */
static inline size_t syringe_hook_arch_tramp_make(uint8_t *bounce_payload, size_t bounce_cap, const void *target,
                                                  size_t *copied_bytes) {
  size_t stolen = 0;
  ...
  if (syringe_memfd >= 0) {
    ssize_t rd = pread(syringe_memfd, prologue_buf, TRAMP_STOLEN_MAX, (off_t)(uintptr_t)target);
  ...
  while (stolen < TRAMP_JMP_SZ) {
    ...
    if (insn_len == 0) {
      SYRINGE_HOOK_LOG("tramp_make: unknown opcode at +%zu (target=%p)", stolen, target);
      return 0;
    }
    if (stolen + insn_len > TRAMP_STOLEN_MAX) {
      SYRINGE_HOOK_LOG("tramp_make: stolen prologue exceeds %d bytes", TRAMP_STOLEN_MAX);
      return 0;
    }
  ...
  syringe_hook_arch_build_jmp(bounce_payload + stolen, (void *)((uintptr_t)target + stolen));
...
/* Backward-compat alias */
static inline size_t syringe_hook_tramp_make(uint8_t *bounce_payload, size_t bounce_cap, const void *target,
                                             size_t *copied_bytes) {
  return syringe_hook_arch_tramp_make(bounce_payload, bounce_cap, target, copied_bytes);
}
...
static inline int syringe_hook_arch_atomic_patch_jmp(void *target, const uint8_t jmp[TRAMP_JMP_SZ]) {
  uint8_t buf[16] = {0};
...
/* Backward-compat alias */
static inline int syringe_hook_atomic_patch_jmp(void *target, const uint8_t jmp[TRAMP_JMP_SZ]) {
