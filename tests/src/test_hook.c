...
static void check_ptr(void *ptr, int expect_null, const char *expr) {
  ...
  if (is_null == expect_null) {
    ...
    tests_fail++;
    fprintf(stderr, "FAIL: %s:%d: expected %s, got %s\n", __FILE__, __LINE__, expect_null ? "NULL" : "non-NULL",
...
static void test_install_remove_cycle(void) {
  ...
  void *target = mmap(NULL, getpagesize(), PROT_READ | PROT_WRITE | PROT_EXEC, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  ...
  void *orig = NULL;
  int r = syringe_hook_install_addr("__test_target", target, (void *)hook_noop_for_cycle, &orig);
   ...
   * NOP = 14. */
  CHECK_EQ(0, memcmp(target, prologue, 14), "target prologue (first 14 bytes) restored after remove");
...
static void test_duplicate_install(void) {
  ...
  static const uint8_t prologue[] = {0xF3, 0x0F, 0x1E, 0xFA, 0x55, 0x48, 0x89, 0xE5, 0xC3, 0x0F, 0x1F, 0x44,
                                     0x00, 0x00, 0x0F, 0x1F, 0x44, 0x00, 0x00, 0x0F, 0x1F, 0x44, 0x00, 0x00};
  void *target = mmap(NULL, getpagesize(), PROT_READ | PROT_WRITE | PROT_EXEC, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  if (target == MAP_FAILED) {
    printf("  SKIP: mmap failed\n");
    return;
  ...
  int r = syringe_hook_install_addr("__test_dup", target, (void *)hook1_for_dup, &orig1);
  CHECK_EQ(1, r, "first install_addr OK");

  int r2 = syringe_hook_install_addr("__test_dup", target, (void *)hook2_for_dup, &orig2);
...
static void test_tramp_install_writable(void) {
  ...
  CHECK_EQ(0, memcmp(target, prologue, sizeof(prologue)), "target prologue restored after remove");
  munmap(target, getpagesize());
#else
...
static int hook_mul(int a, int b) { return a * b; /* ignore orig — we just want to see the hook fire */ }

static void test_install_addr(void) {
  printf("\n[install_addr]\n");
#ifndef SYRINGE_HOOK_NO_HELPERS

  uint8_t target_code_v2[] = {/* 0:  endbr64             */ 0xF3,
                              0x0F,
                              0x1E,
                              0xFA,                               /* 4 bytes */
  ...
  int n = syringe_hook_install_addr("target_add", (void *)target_add, (void *)hook_mul, (void **)&orig_add_for_test);
...
static void test_tramp_make_rip_relative(void) {
  ...
  size_t copied = 0;
  size_t stolen = syringe_hook_tramp_make(bounce, sizeof(bounce), page, &copied);
  ...
  CHECK_EQ((uintptr_t)((uint8_t *)page + stolen), (uintptr_t)jmp_dst, "bounce tail JMP returns to target+stolen");
