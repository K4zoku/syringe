...
static inline uint32_t emit_stp_pre(int rt, int rt2) {
  ...
  return 0xA9800000                                             /* base: stp ..., [sp, #0]! pre-indexed */
         ...
         | ((uint32_t)(rt2 & 0x1F) << 10) | ((uint32_t)31 << 5) /* Rn = sp */
         | (uint32_t)(rt & 0x1F);
}
...
static inline uint32_t emit_adr_x0(int32_t offset) {
  ...
  return 0x10000000 | ((uint32_t)immlo << 29) | ((uint32_t)immhi << 5) | 0; /* Rd = x0 */
}

/* mov x1, #imm16  (movz, 16-bit immediate, no shift)
 * Encoding: 110100101 00 imm16 Rd
 */
static inline uint32_t emit_movz_x1(uint16_t imm16) { return 0xD2800000 | ((uint32_t)imm16 << 5) | 1; /* Rd = x1 */ }
 ...
 *   Rn = 16
 */
static inline uint32_t emit_blr_x16(void) { return 0xD63F0200; }

/* brk #0  — breakpoint trap (used by injector to catch the shellcode) */
static inline uint32_t emit_brk_0(void) { return 0xD4200000; }

/* ret  — return to address in x30 (link register) */
static inline uint32_t emit_ret(void) { return 0xD65F03C0; }
...
#define EMIT32(v)                                                                                                      \
  ...
  EMIT32(0xA9800000 | ((uint32_t)(-2 & 0x7F) << 15) | ((uint32_t)31 << 10) /* Rt2 = xzr */
         | ((uint32_t)31 << 5)                                             /* Rn = sp */
         | 28);                                                            /* Rt = x28 */
  ...
  /* ldp x28, xzr, [sp], #16 */
  EMIT32(0xA8C00000 | ((uint32_t)2 << 15) | ((uint32_t)31 << 10) /* Rt2 = xzr */
         | ((uint32_t)31 << 5)                                   /* Rn = sp */
         | 28);                                                  /* Rt = x28 */
  ...
  adr_insn = (adr_insn & 0x9F00001F) | ((uint32_t)immlo << 29) | ((uint32_t)immhi << 5);
...
unsigned long syringe_arch_entry_skip(void) { return 0; /* aarch64 has no ptrace restart quirk */ }

unsigned int syringe_arch_jump_slot_type(void) { return R_AARCH64_JUMP_SLOT; }

unsigned int syringe_arch_glob_dat_type(void) { return R_AARCH64_GLOB_DAT; }
