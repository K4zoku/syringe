...
static inline int syringe_hook_dotnet_debug(void) {
    ...
    if (!checked) { checked = 1; enabled = getenv("SYRINGE_HOOK_DEBUG") && getenv("SYRINGE_HOOK_DEBUG")[0] != '\0'; }
    return enabled;
}
...
static inline uint16_t syringe_dotnet_get_u16le(const uint8_t *buf) {
...
static inline uint8_t *syringe_dotnet_build_attach_msg(const char *profiler_path,
...
static inline int syringe_dotnet_send_recv(const char *socket_path,
    ...
    if (path_len >= sizeof(addr.sun_path)) {
        if (syringe_hook_dotnet_debug()) fprintf(stderr, "[*] [dotnet] socket path too long: %s\n", socket_path);
        close(fd);
        return -1;
    }
    ...
    if (total_size < 20) total_size = 20;
    if (total_size > resp_buf_size) total_size = (uint16_t)resp_buf_size;
...
static inline int syringe_dotnet_attach_profiler(pid_t pid,
    ...
    if (syringe_hook_dotnet_debug()) fprintf(stderr, "[*] [dotnet] Found diagnostic socket: %s\n", socket_path);

    size_t msg_len = 0;
    uint8_t *msg = syringe_dotnet_build_attach_msg(profiler_path, client_data,
                                                    client_len, &msg_len);
    if (!msg) {
    ...
    if (syringe_hook_dotnet_debug()) fprintf(stderr, "[*] [dotnet] Sending AttachProfiler (profiler=%s, %zu bytes client_data)\n",
    ...
    free(msg);
    if (rc < 0) return SYRINGE_DOTNET_ERROR;
    ...
    if (resp_cmd == 0x00) {
        if (syringe_hook_dotnet_debug()) fprintf(stderr, "[*] [dotnet] AttachProfiler OK — profiler .so loaded\n");
        return SYRINGE_DOTNET_OK;
    } else if (resp_cmd == 0xFF) {
        uint32_t hresult = 0;
        if (resp_len >= 24) hresult = syringe_dotnet_get_u32le(resp + 20);

        if (hresult == 0x80040154) {
            if (syringe_hook_dotnet_debug()) fprintf(stderr, "[*] [dotnet] Injected OK\n");
            return SYRINGE_DOTNET_OK;
        }
        ...
        if (hresult == 0x8013136A) {
            if (syringe_hook_dotnet_debug()) fprintf(stderr, "[*] [dotnet]   → CORPROF_E_PROFILER_ALREADY_ACTIVE\n");
        } else if (hresult == 0x80131385) {
            if (syringe_hook_dotnet_debug()) fprintf(stderr, "[*] [dotnet]   → Unknown command\n");
        } else if (hresult == 0x80070057) {
            if (syringe_hook_dotnet_debug()) fprintf(stderr, "[*] [dotnet]   → Invalid argument\n");
        } else if (hresult == 0x80131515) {
            if (syringe_hook_dotnet_debug()) fprintf(stderr, "[*] [dotnet]   → Not supported\n");
        }
