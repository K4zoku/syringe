...
static inline void *syringe_hook_arch_read_jmp_target(void *src) {
  ...
  if (p[0] != 0x58000050 || p[1] != 0xD61F0200)
    return NULL;
  void *dst;
 ...
 */
static inline size_t syringe_hook_arch_tramp_make(uint8_t *bounce_payload, size_t bounce_cap, const void *target_in,
                                                  size_t *copied_bytes) {
  ...
  while (stolen < TRAMP_STOLEN_MAX) {
    ...
    else if ((insn & 0xBF000000) == 0x18000000 || (insn & 0xFF000000) == 0x98000000) {
