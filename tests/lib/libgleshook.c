...
static EGLBoolean hook_eglSwapBuffers(EGLDisplay dpy, EGLSurface surface) { return orig_eglSwapBuffers(dpy, surface); }

static void hook_glXSwapBuffers(Display *dpy, GLXDrawable drawable) { orig_glXSwapBuffers(dpy, drawable); }

static void hook_SDL_GL_SwapWindow(void *window) { orig_SDL_GL_SwapWindow(window); }

static int hook_wl_surface_commit(struct wl_proxy *surface) { return orig_wl_surface_commit(surface); }
 ...
 */
#define TRY_HOOK(sym, hook_fn, orig_ptr)                                                                               \
  do {                                                                                                                 \
    void *_real = dlsym(RTLD_DEFAULT, #sym);                                                                           \
    if (_real) {                                                                                                       \
      int _n = syringe_hook_install(#sym, (void *)(hook_fn), (void **)&(orig_ptr));                                    \
      if (_n == 0) {                                                                                                   \
...
__attribute__((constructor)) static void sample_load(void) {
...
__attribute__((destructor)) static void sample_unload(void) {
